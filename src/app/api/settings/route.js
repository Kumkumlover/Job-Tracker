...
async function GET(req) {
    try {
        const userId = await (0, prisma_1.getDefaultUserId)();
        let profile = await prisma_1.prisma.profileContext.findUnique({
            where: { userId },
        });
        ...
        if (!profile) {
            profile = await prisma_1.prisma.profileContext.create({
                data: {
                    userId,
...
async function POST(req) {
    try {
        ...
        const updated = await prisma_1.prisma.profileContext.upsert({
            where: { userId },
            update: {
        ...
        if (body.hunterKey !== undefined)
            userUpdateData.hunterKey = body.hunterKey;
        if (body.apolloKey !== undefined)
            userUpdateData.apolloKey = body.apolloKey;
        if (body.serperKey !== undefined)
            userUpdateData.serperKey = body.serperKey;
        if (body.geminiKey !== undefined)
            userUpdateData.geminiKey = body.geminiKey;
        if (body.tavilyKey !== undefined)
            userUpdateData.tavilyKey = body.tavilyKey;
        if (body.exaKey !== undefined)
            userUpdateData.exaKey = body.exaKey;
        ...
        return server_1.NextResponse.json({ success: true, profile: updated });
    }
    catch (err) {
